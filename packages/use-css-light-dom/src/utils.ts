import { Sheet } from "atomico";

const scaleSelector = (selector: string) =>
	selector.replace(/([^\w\_\-])/g, "\\$1");
	...
	for (let i = 0; i < cssRules.length; i++) {
		...
		if (rule instanceof CSSStyleRule) {
			...
			rules.push(
				cssText.replace(
					/\s+([^\s{]+)/,
					" $1-" + scaleSelector(scopeSelector),
				),
			);
		} else if (rule instanceof CSSMediaRule) {
			const { conditionText } = rule;
			rules.push(
				`@media ${conditionText}{${getRules(rule, scopeSelector)}}`,
			);
		}
	}
	return rules;
...
export function getSheet(style: Sheet) {
	...
	style.cloneNode = (deep?: boolean): HTMLElement => {
