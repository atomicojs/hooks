...
const AShowcase = ({
  ...
  const propsMove = useEventMove((type, range) => {
    ...
    if (type == "move") {
      ...
      const minWidth = refSelect.current ? refSelect.current.clientWidth + 20 : 50;
  ...
  useEffect(() => {
    (async () => {
      setState(state => state.loading ? state : { ...state,
        loading: false
      });
      const {
        render,
  ...
  return createElement("host", {
    shadowDom: true
  }, createElement("style", null, style), state.cases.length > 1 && createElement("select", {
    ref: refSelect,
    ...
    value: label
  }, label))), createElement("header", null), createElement("section", {
...
customElement("a-showcase", AShowcase);
...
//# sourceMappingURL=a-showcase.test.js.map
